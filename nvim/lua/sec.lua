...
source.complete = function(self, request, callback)
	...
	local stdmv = string.sub(line, cursor_pos - 6, cursor_pos)
	local cursor_col = vim.fn.col(".")
	local table_name = line:sub(1, cursor_col):match(self.regex_pattern)

	if vim.startswith(string.lower(stdmv), "secmv.") then
		trigger = true
	end
...
cmp.register_source("secmv", source)
